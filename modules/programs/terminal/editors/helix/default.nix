...
in {
  options.${namespace}.programs.terminal.editors.helix = {
    enable = mkEnableOption "Whether to enable Helix editor.";
  };

  config = mkIf cfg.enable {
    hjem.users.${user}.rum.programs.helix= {
    programs.helix = {
      enable = true;
        ...
        language-server = {
          ...
          nixd = {
            ...
            config.nixd.options = let
              options = "(builtins.getFlake \"${inputs.self}\").nixosConfigurations.loki.options";
            in {
              nixos.expr = options;
